        ...
        <section class="bp_content">
            ...
            <div class="bpclause bp_line">
                <div class="bp_checked" @click="isGreen=!isGreen">
                    <img v-if="isGreen" src="../../static/img/checkedicon.png" alt="" srcset="">
                </div>
                <p class="bp_clause_content"><span style="color: #02ad4d;">已确认转账信息和银行预留信息一致。</span><br>我已查阅<a @click="isStatement=!isStatement">《保费自动转账授权声明》</a>，并同意授权从该银行账户自动扣取各期保费，首期保费缴纳成功日期为当月1日至24日的保单，续期保险费扣费日则为从下月开始的每月15日；首期保费缴纳成功日期为当月25日至31日的保单，续期保险费扣费日为从隔月开始的每月15日。</p>
            </div>
        </section>
        <div class="bp_btn">
...
export default {
  ...
  created() {
    ...
    this.certList({
      successCallback: result => {
        let recordData = JSON.parse(window.localStorage.getItem("recordData"));
        for (let item of result) {
          console.log(item)
          item.callback = this.certFn;
          item.name = item.cert_desc;
          if(recordData.cert_type == item.cert_desc){
  ...
  mounted() {
    let recordData = JSON.parse(window.localStorage.getItem("recordData"));
    this.bplists[0].listright = recordData.orderNumber;
    this.bplists[2].listright = recordData.money;
    this.bplistsTwo[0].listcontent = recordData.userName;
    this.bplistsTwo[4].listcontent = recordData.cert_code;  //证件号码
    this.bplistsTwo[5].listcontent = recordData.tel;  //手机号码
  ...
  methods: {
    ...
    fourJqFnOne() {
      ...
      let FourData = {
        ...
        CERT_CODE: this.bplistsTwo[4].listcontent, //证件号码
      ...
      this.fourJqFn({
        ...
        failCallback: (result, resultCode) => {
          ...
          // this.$refs.alertFn.isworngFn(result, "inputVcode");
          if(resultCode == '2'){
    ...
    payFn() {
      ...
      if (this.isNeesAuth == "1") {
        payData = {
          ...
          BANKCODE: this.bankCode,
          CARDNUMBER: this.bplistsTwo[2].listcontent,
          APPLICATIONCODE: JSON.parse(window.localStorage.getItem("recordData")).applicationCode
          // CERT_TYPE: this.certCode, //证件类型
          // CERT_CODE: this.bplistsTwo[4].listcontent, //银行卡号
          // PHONENUMER: this.bplistsTwo[5].listcontent //手机号码
          // openId: openidList.openId, //手机号码
      ...
      //   BANKCODE: this.bankCode,
      //   CARDNUMBER: this.bplistsTwo[2].listcontent,
      //   APPLICATIONCODE: JSON.parse(window.localStorage.getItem("recordData"))
      //     .applicationCode
      // };
      payData = JSON.stringify(payData);
      let payListDataTwo = new FormData();
      payListDataTwo.append("payData", payData);
      ...
      this.tepToPayTwo({
        ...
        successCallback: result => {
          ...
          // } else {
            this.$router.push({ path: "/paypageTwo" });
          // }
        },
        failCallback: result => {
          toast1.clear();
