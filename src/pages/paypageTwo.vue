...
export default {
  ...
  created(){
    let recordData = JSON.parse(
      window.localStorage.getItem("recordData")
    );
    ...
    this.bplists[1].listright = recordData.orderNumber
    this.bplists[2].listright = recordData.time
    this.bplistsTree[0].listright = '¥ ' + recordData.money
    this.clientName = recordData.userName
    if(recordData.gender == 'M'){
      this.clientSex = '先生'
    }else{
      this.clientSex = '女士'
    }
